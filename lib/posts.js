...
export function getAllCategories() {
  ...
  const seen = new Set(posts.map((post) => post.frontmatter.category));
  return Array.from(seen);
}

export function getPostsBySection(sectionSlug) {
  return getAllPosts().filter(
    (post) => getCategoryByName(post.frontmatter.category).slug === sectionSlug
  );
}

export function getSectionsWithPosts() {
  const allPosts = getAllPosts();
  return categories
    .map((section) => ({
      ...section,
      posts: allPosts.filter(
        (post) => getCategoryByName(post.frontmatter.category).slug === section.slug
      ),
    }))
